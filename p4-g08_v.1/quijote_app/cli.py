...
def index_command(
    ...
    cache_path: Optional[Path] = typer.Option(
        None,
        "--cache-path",
        help="Ruta de cachÃ© del Ã­ndice (pickle).",
    ),
    min_words: int = typer.Option(
        MIN_WORDS_PER_PASSAGE,
        "--min-words",
        min=1,
        help="MÃ­nimo de palabras por pasaje indexable.",
    ),
    force: bool = typer.Option(
        False,
        "--force",
        help="Reconstruye Ã­ndice aunque exista cachÃ© vÃ¡lida.",
    ),
    no_cache: bool = typer.Option(
        False,
        "--no-cache",
        help="No leer ni guardar cachÃ©.",
    ),
) -> None:
    """Construye o actualiza el Ã­ndice del corpus."""
    try:
    ...
    if no_cache:
        typer.echo("CachÃ©: desactivada")
    else:
        status = "reutilizada" if from_cache else "actualizada"
        typer.echo(f"CachÃ©: {effective_cache} ({status})")

    typer.echo(f"Pasajes indexados: {index.metadata.passage_count}")
    typer.echo(f"CapÃ­tulos detectados: {index.metadata.chapter_count}")
    typer.echo(f"Partes detectadas: {index.metadata.part_count}")


@app.command("search")
def search_command(
    query: str = typer.Argument(..., help="TÃ©rmino o frase a buscar."),
    source: Optional[Path] = typer.Option(
        None,
        "--source",
        "-s",
    ...
    limit: int = typer.Option(
        ...
        min=1,
        max=100,
        help="MÃ¡ximo de resultados a mostrar.",
    ),
    chapter: Optional[str] = typer.Option(
        None,
        "--chapter",
        help="Filtro textual por capÃ­tulo (normalizado).",
    ),
    cache_path: Optional[Path] = typer.Option(
        None,
        "--cache-path",
    ...
    max_chars: int = typer.Option(
        ...
        min=120,
        max=1200,
        help="Longitud mÃ¡xima de pasaje mostrado.",
    ),
) -> None:
    """Busca y devuelve pasajes relevantes del corpus usando lemas."""
    if not query.strip():
        typer.secho("Error: la consulta no puede estar vacÃ­a.", fg=typer.colors.RED, err=True)
    ...
    _print_results(query=query, results=results, max_chars=max_chars)
...
def interactive_command(
    ...
    limit: int = typer.Option(
        ...
        min=1,
        max=100,
        help="MÃ¡ximo de resultados por consulta.",
    ),
    chapter: Optional[str] = typer.Option(
        None,
        "--chapter",
        help="Filtro textual inicial por capÃ­tulo (normalizado).",
    ),
    cache_path: Optional[Path] = typer.Option(
        None,
        "--cache-path",
    ...
    rebuild: bool = typer.Option(
        False,
        "--rebuild",
        help="Reindexa al iniciar la sesiÃ³n.",
    ),
    max_chars: int = typer.Option(
        SNIPPET_MAX_CHARS,
        "--max-chars",
    ...
    """Lanza una sesiÃ³n interactiva de consultas hasta escribir exit."""
...
def _run_interactive_session(
    ...
    welcome.append("Buscador clasico por lemas del Quijote para consultas en terminal.\n")
    welcome.append("No invento respuestas: recupero texto real del corpus por coincidencia de lemas.\n\n")
    ...
    while True:
        try:
            raw = console.input("[bold cyan]quijote> [/bold cyan]")
...
def stats_command(
    ...
    rebuild: bool = typer.Option(
        False,
        "--rebuild",
        help="Forzar reconstrucciÃ³n del Ã­ndice.",
    ),
) -> None:
    """Muestra estadÃ­sticas del Ã­ndice y del corpus."""
    try:
        resolved_source = resolve_source_path(source)
        index, from_cache, effective_cache = load_or_build_index(
            source=resolved_source,
    ...
    typer.echo("EstadÃ­sticas del Ã­ndice")
    typer.echo("----------------------")
    ...
    typer.echo(f"CapÃ­tulos detectados (metadato): {index.metadata.chapter_count}")
    typer.echo(f"CapÃ­tulos con pasajes: {unique_chapters}")
    typer.echo(f"Partes detectadas: {index.metadata.part_count}")
    typer.echo(f"Media de palabras por pasaje: {avg_words:.2f}")
    typer.echo(f"Fecha construcciÃ³n (UTC): {index.built_at_iso}")
    if no_cache:
        typer.echo("CachÃ©: desactivada")
    else:
        cache_status = "reutilizada" if from_cache else "actualizada"
        typer.echo(f"CachÃ©: {effective_cache} ({cache_status})")
...
def chapters_command(
    ...
    limit: int = typer.Option(
        ...
        "-l",
        min=1,
        help="NÃºmero mÃ¡ximo de capÃ­tulos a listar.",
    ),
) -> None:
    """Lista capÃ­tulos detectados y nÃºmero de pasajes por capÃ­tulo."""
    try:
        resolved_source = resolve_source_path(source)
    ...
    counter = Counter((p.chapter or "Sin capÃ­tulo detectado") for p in index.passages)
    typer.echo(f"CapÃ­tulos listados: {min(limit, len(counter))}/{len(counter)}")
    typer.echo("")
...
def _print_results(query: str, results: list[SearchResult], max_chars: int) -> None:
    if not results:
        console.print("[yellow]No se encontraron pasajes para esa consulta.[/yellow]")
        return

    for idx, result in enumerate(results, start=1):
        chapter_label = _shorten(result.passage.chapter or "Sin capÃ­tulo detectado", 120)
