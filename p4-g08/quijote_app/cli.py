...
app = typer.Typer(
    ...
    help="Buscador literal del Quijote (recuperación de pasajes reales).",
)
...
def index_command(
    ...
    cache_path: Optional[Path] = typer.Option(
        None,
        "--cache-path",
        help="Ruta de caché del índice (joblib).",
    ),
...
def search_command(
    ...
    """Busca y devuelve pasajes literales del corpus."""
    if not query.strip():
...
def _run_interactive_session(
    ...
    welcome.append("Buscador literal del Quijote para consultas en terminal.\n")
    welcome.append("No invento respuestas: recupero texto real del corpus.\n\n")
...
def stats_command(
    ...
    """Muestra estadísticas del índice y del corpus."""
    try:
        resolved_source = resolve_source_path(source)
        index, from_cache, effective_cache = load_or_build_index(
            source=resolved_source,
...
def chapters_command(
    ...
    limit: int = typer.Option(
        ...
        "-l",
        min=1,
        help="Número máximo de capítulos a listar.",
    ),
) -> None:
    """Lista capítulos detectados y número de pasajes por capítulo."""
    try:
        resolved_source = resolve_source_path(source)
    ...
    typer.echo(f"Capítulos listados: {min(limit, len(counter))}/{len(counter)}")
    typer.echo("")
...
def _print_interactive_help() -> None:
    ...
    help_table.add_row(
        "consulta libre",
        "Busca pasajes literales por palabra o frase",
