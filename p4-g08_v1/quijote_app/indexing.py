...
import ollama  # Importación permitida en la rúbrica
...
# Modelo de embeddings recomendado por defecto. Ligero y muy rápido.
...
def _generate_embeddings(passages: list[Passage], model: str = DEFAULT_EMBEDDING_MODEL) -> None:
    ...
    for passage in passages:
        try:
            # Pedimos a Ollama que transforme el texto normalizado en un vector de números
            response = ollama.embeddings(model=model, prompt=passage.text_normalized)
...
def load_or_build_index(
    ...
    if use_cache and not force_rebuild and effective_cache.exists():
        ...
        has_embeddings = bool(cached_index.passages and cached_index.passages[0].embedding is not None)
